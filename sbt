...
mainFiltered () {
  ...
  echoLine () {
    local line="$1"
    local line1="$(echo "$line" | gsed -r 's/\r\x1BM\x1B\[2K//g')"       # This strips the OverwriteLine code.
    local line2="$(echo "$line1" | gsed -r 's/\x1B\[[0-9;]*[JKmsu]//g')" # This strips all codes - we test regexes against this.
